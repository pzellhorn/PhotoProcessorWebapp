...
import PhotoCard, { formatTimestamp } from "../components/PhotoCard";
...
export default function GalleryPage() {
  ...
  const [progress, setProgress] = useState({ done: 0, total: 0, fraction: 0 });
  ...
  const upload = {
    ...
    start: (files) => {
      ...
      uploadMutation.mutate({
        files,
        onProgress: (done, total, fraction = 0) =>
          setProgress({ done, total, fraction }),
      });
    },
  };
  ...
  return (
    ...
    >
      {isDragging && (
        <div className="dropzone-overlay">Drop photos to upload</div>
      )}
        ...
        <div>
          ...
          {!isSearching && data && (
            <span className="muted">
              {data.totalCount} photo{data.totalCount === 1 ? "" : "s"}
            </span>
          )}
        </div>
      ...
      {isSearching ? (
        <>
          ...
          {search.isError && (
            <p className="error-text">{search.error.message}</p>
          )}
          {search.data && (
            <>
              <p className="muted">
              ...
              <div className="photo-grid">
                {search.data.map((item) => (
                  <PhotoCard
                    ...
                    to={
                      item.parentMediaId
                        ? `/photos/${item.parentMediaId}?t=${Math.floor((item.timestampMs ?? 0) / 1000)}`
                        : undefined
                    }
                    badgeLabel={
                      item.parentMediaId
        ...
        <>
          ...
          {data && data.items.length === 0 && (
            <p className="muted">
              No photos yet — drop some here to get started.
            </p>
          )}
